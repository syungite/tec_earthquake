...
def anomaly_plotting(c_t, plotting_path, title, fixed):
    # Plotting graph
    if len(c_t) > 0:
        ...
        for i in range(0, len(c_t), 30):  # Every 30 time points, which corresponds to 15 minutes
            ax.axvline(time_labels[i], color='white', linestyle='-', alpha=0.5, zorder=2)
